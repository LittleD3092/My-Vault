...
int SevenSeg__printNumUpper(SevenSeg* self, int num)
{
	...
	{
		int digit = num % 10;
		send_7seg(self->gpio, self->DIN, self->CS, self->CLK, SEG_ADDRESS_DIGIT_0+i, SEG_DATA_DECODE_0+digit);
		num /= 10;
	}
	if(num < 0) send_7seg(self->gpio, self->DIN, self->CS, self->CLK, SEG_ADDRESS_DIGIT_7, SEG_DATA_DECODE_DASH);
