...
int main(){
	...
	if(init_led(LED_gpio2, LED1_pin) != 0 || init_led(LED_gpio1, LED2_pin) != 0 || init_led(LED_gpio1, LED3_pin) != 0 || init_led(LED_gpio1, LED4_pin) != 0 ||
	...
	while(1){
		...
		for(int a=0;a<4;a++){
			// Active Low
			if((led_data>>(a+1))&0x1){
				reset_gpio(a == 0 ? LED_gpio2 : LED_gpio1, leds[a]);
			}
			else{
				set_gpio(a == 0 ? LED_gpio2 : LED_gpio1, leds[a]);
			}
		}
